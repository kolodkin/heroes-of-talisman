...
const Board = ({ username, playerData, playing, selected_character }) => {
  ...
  const disconnected =
    playerData.status === "disconnected" ? (
      <div
        className={classNames(styles["disconnected-card"], "flex justify-center items-center absolute w-full h-full")}
      >
        {" "}
        {lang.player_card.disconnected}
      </div>
    ) : null;
  return (
...
const ActionBoard = ({ username, gamename, game, sendAction, handleLeave }) => {
  ...
  switch (stage) {
    case "character_select":
      content = (
        <CharacterSelect
          characters={game.players[username].characters}
          sendAction={sendAction}
          active={game.active}
          selectedCharacter={game.stage_meta?.selected}
        />
      );
      break;
    case "card_draw":
      content = (
        <DrawCard
          sendAction={sendAction}
          card={game.stage_meta.card}
          drawn={game.stage_meta.drawn}
          active={game.active}
        />
      );
      break;
  ...
  return (
    <div className={classNames(styles["action-board"], "relative", "p-4")}>
      <div className="absolute top-2 end-2 flex space-x-2">
        ...
        <div className={styles["disconnect-button"]} onClick={handleLeave} title="צא מהמשחק">
          <span>X</span>
        </div>
      </div>
